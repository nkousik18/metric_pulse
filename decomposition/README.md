...
| `fetch_dimension_metrics` | `(dimension, current_date, previous_date, metric_col='total_revenue', dimension_config=DIMENSION_TABLES, connection_factory=get_connection) -> pd.DataFrame` | Validates both dates, then runs a single query: two CTEs (`current_day`, `previous_day`) `FULL OUTER JOIN`ed on segment, so a segment with data on only one of the two dates still appears (missing side defaults to `0` via `COALESCE`). Opens/closes its own connection per call via `connection_factory()` — see Gotchas. `dimension_config`/`connection_factory` are additive (Phase 2, `docs/scoping.md` §6.2) — every existing call site omits both and gets byte-identical Redshift behavior; an onboarded dataset passes its generated `dimension_config` and a `duckdb.connect(path)` callable instead. |
| `fetch_detail_metrics` | `(dimension, segment, current_date, previous_date, metric_col='total_revenue', dimension_config=DIMENSION_TABLES, connection_factory=get_connection) -> pd.DataFrame` | Same shape as `fetch_dimension_metrics`, grouped by `detail_col` and filtered to one `segment_col` value (Phase 1's drill-down). Same additive `dimension_config`/`connection_factory` pair. |
...
| `summarize_dimension` | `(df_analyzed: pd.DataFrame) -> Dict` | Pure function. Builds one dimension's `decompose_metric()` result entry (totals + top 5 `top_contributors`) from an already-`calculate_contribution()`-analyzed DataFrame. |
| `decompose_metric` | `(current_date, previous_date, metric_col='total_revenue', dimension_config=DIMENSION_TABLES, connection_factory=get_connection) -> Dict` | Loops `dimension_config`'s keys (all 3 Olist dimensions by default), calls `fetch_dimension_metrics`/`calculate_contribution`/`summarize_dimension` for each. A per-dimension failure is caught and stored as `{'error': str(e)}` in that dimension's slot rather than aborting the whole call. Same additive `dimension_config`/`connection_factory` pair, passed straight through to `fetch_dimension_metrics`. |
...
| `get_comparison_dates` | `(target_date: str = None, table_name='staging.fact_daily_metrics', connection_factory=get_connection) -> tuple[str, str]` | No `target_date` → most recent 2 distinct dates in `table_name`. With `target_date` → most recent 2 dates `<= target_date`. Raises `ValueError` if fewer than 2 dates exist. `table_name`/`connection_factory` are additive, same reasoning as above — not named in `docs/scoping.md` §6.2's own additive-parameters list, but functionally required for this function to work against an onboarded dataset at all (an onboarding-specific finding, not something the design doc's illustrative snippet covered). |
...
- SQL is built via f-string interpolation of `metric_col`, `config['table']`, `config['segment_col']` — these come from the hardcoded `DIMENSION_TABLES` dict, not user input, so only the *date* args need (and get) `_validate_date`. If you ever make `metric_col` user-controllable, it needs the same treatment. The same trust model extends to Phase 2's `dimension_config`/`table_name` parameters — they come from `onboarding/codegen.py`'s generated config, not raw external input.
- **DuckDB compatibility (Phase 2, `docs/scoping.md` §6.2/§6.7) is real, not assumed** — confirmed directly in a REPL before wiring anything: `pd.read_sql(query, conn)`, `conn.cursor()`, and `conn.close()` all work against a `duckdb.connect(...)` connection exactly like they do against `redshift_connector`'s. The `FULL OUTER JOIN`/`COALESCE` SQL these functions build runs unchanged on DuckDB.
