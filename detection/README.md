...
|----------|-----------|---------------|
| `fetch_daily_metrics` | `(lookback_days: int = 30) -> pd.DataFrame` | `SELECT metric_date, order_count, customer_count, total_revenue, avg_order_value FROM staging.fact_daily_metrics ORDER BY metric_date DESC LIMIT N`. Always pulls all 4 metric columns regardless of which one you'll analyze. Opens/closes its own Redshift connection. |
...
| `run_detection` | `(metric='total_revenue', lookback_days=None, threshold=None) -> dict` | Top-level orchestration function. `lookback_days=None` → reads `LOOKBACK_DAYS` env (default `30`). Returns `{'status': 'no_data', 'anomalies': []}` early if the query returns nothing — note this shape is *different* from the normal return (no `metric`/`statistics` keys), so callers must check `status` before assuming the full shape exists. |
